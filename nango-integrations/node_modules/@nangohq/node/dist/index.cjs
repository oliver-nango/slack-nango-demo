...
function getUserAgent(userAgent) {
  ...
  const osName = import_os.default.platform().replace(" ", "_");
  const osVersion = import_os.default.release().replace(" ", "_");
  return `nango-node-client/${NANGO_VERSION} (${osName}/${osVersion}; node.js/${nodeVersion})${userAgent ? `; ${userAgent}` : ""}`;
}
function addQueryParams(url, queries) {
  if (!queries) {
    return;
  }
...
var defaultHttpsAgent = new import_node_https.default.Agent({ keepAlive: true });
var Nango = class {
  serverUrl;
  secretKey;
  connectionId;
   ...
   */
  async triggerSync(providerConfigKey, syncs, connectionId, syncMode) {
  ...
  async waitForConnection(integrationKey, userId, options) {
    ...
    const initialDelayMs = 1e4;
    const delayBetweenAttemptsMs = 2e3;
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const response = await this.listConnections(void 0, void 0, { endUserId: userId });
  ...
  verifyIncomingWebhookRequest(body, headers) {
    ...
    const expectedSignature = import_node_crypto.default.createHmac("sha256", this.secretKey).update(body).digest("hex");
    ...
    try {
      return import_node_crypto.default.timingSafeEqual(Buffer.from(expectedSignature, "hex"), Buffer.from(actualSignature, "hex"));
    } catch {
      return false;
    }
  }
...
Nango.prototype.verifyWebhookSignature = import_node_util.default.deprecate(
