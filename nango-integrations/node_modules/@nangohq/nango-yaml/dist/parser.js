import { ParserErrorDuplicateEndpoint, ParserErrorDuplicateModel, ParserErrorMissingId, ParserErrorModelIsLiteral, ParserErrorModelNotFound } from './errors.js';
import { ModelsParser, getRecursiveModelNames } from './modelsParser.js';
export class NangoYamlParser {
    yaml;
    raw;
    parsed;
    modelsParser;
    errors = [];
    warnings = [];
    constructor({ raw, yaml }) {
        this.yaml = yaml;
        this.raw = raw;
        this.modelsParser = new ModelsParser({ raw: raw.models });
    }
    getModelForOutput({ rawOutput, name, type, integrationName }) {
        if (!rawOutput) {
            return null;
        }
        const models = [];
        ...
        for (const modelOrType of output) {
            ...
            const anonModel = { name: anon, fields: parsed, isAnon: true };
    ...
    postParsingValidation() {
        ...
        for (const integration of this.parsed.integrations) {
                ...
                if (sync.output) {
                    for (const output of sync.output) {
                        ...
                        if (output.startsWith('Anonymous') && !model.fields[0]?.union) {
                            this.warnings.push(new ParserErrorModelIsLiteral({ model: model.fields[0].value, path: [integrationName, 'syncs', sync.name, '[output]'] }));
                            continue;
                        }
                    }
                }
                if (sync.input) {
                    if (sync.input.startsWith('Anonymous')) {
                        ...
                        if (!model.fields[0]?.union) {
                            this.warnings.push(new ParserErrorModelIsLiteral({ model: model.fields[0].value, path: [integrationName, 'syncs', sync.name, '[input]'] }));
                        }
                    }
                    // We register the model usage and recursively too
            ...
            for (const action of integration.actions) {
                const usedModelsAction = new Set();
                if (action.output) {
                    for (const output of action.output) {
                        ...
                        const model = this.modelsParser.get(output);
                        if (output.startsWith('Anonymous') && !model.fields[0]?.union) {
                            this.warnings.push(new ParserErrorModelIsLiteral({
                                model: model.fields[0].value,
