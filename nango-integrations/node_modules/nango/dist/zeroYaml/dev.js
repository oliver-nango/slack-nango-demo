...
export async function dev({ fullPath, debug }) {
 ...
 */
function manualWatch({ fullPath, debug }) {
    const watchPath = ['./**/*.ts'];
    ...
    function bufferOrDisplayError(filePath, msg, line) {
        if (isReady) {
    ...
    async function onAddOrUpdate(filePath) {
        let spinner;
 ...
 */
function typescriptWatchSimple({ fullPath, debug }) {
    let hadError = false;
    ...
    const host = ts.createWatchCompilerHost(path.join(fullPath, 'tsconfig.json'), tsconfig, ts.sys, ts.createSemanticDiagnosticsBuilderProgram, tsDiagnosticToText(fullPath), reportWatchStatusChanged);
...
class DependencyGraph {
     ...
     */
    findAllDependents(file, visited = new Set()) {
