...
export async function deploy({ fullPath, options, environmentName }) {
...
async function createPackage({ parsed, fullPath, debug, version = '', optionalIntegrationId, optionalSyncName, optionalActionName }) {
    printDebug('Packaging', debug);
    const postData = [];
    const onEventScriptsByProvider = optionalActionName || optionalSyncName ? undefined : []; // only load on-event scripts if we're not deploying a single sync or action
    ...
    for (const integration of parsed.integrations) {
        ...
        if (onEventScriptsByProvider) {
            const scripts = [];
            for (const event of Object.keys(onEventScripts)) {
 ...
 */
async function loadScriptFiles({ fullPath, scriptName, providerConfigKey, type }) {
 ...
 * Load bundled file
 */
async function loadScriptJsFile({ scriptName, providerConfigKey, fullPath, type }) {
 ...
 */
async function loadScriptTsFile({ fullPath, scriptName, providerConfigKey, type }) {
 ...
 */
async function postConfirmation({ body }) {
 ...
 */
async function postDeploy({ body }) {
    ...
    try {
        ...
        return Ok(`Successfully deployed the functions: \r\n${nameAndVersions
            .map((row) => {
            return `- ${row}`;
        })
            .join('\r\n')}`);
 ...
 */
async function handleConfirmation({ autoconfirm, allowDestructive, confirmation }) {
    const { newSyncs, updatedSyncs, deletedSyncs, newActions, updatedActions, deletedActions, newOnEventScripts, updatedOnEventScripts, deletedOnEventScripts, deletedModels } = confirmation;
    ...
    if (newSyncs.length > 0 || deletedSyncs.length > 0) {
        ...
        for (const sync of newSyncs) {
            const syncMessage = sync.connections === 0 || !sync.auto_start
        ...
        for (const sync of deletedSyncs) {
            const syncMessage = sync.connections === 0 ? chalk.gray('(0 impacted connections)') : chalk.gray(`(${chalk.red(`${sync.connections} impacted connections`)})`);
    ...
    if (deletedModels.length > 0) {
        ...
        console.log(chalk.red("WARNING: Renaming a model is the equivalent of deleting the old model and creating a new one. Records from the old model won't be transferred to the new model. Consider running a full sync to transfer records."));
    }
    ...
    if (isCI) {
        console.log(chalk.red(`Syncs/Actions were not deployed. Confirm the deploy by passing the --auto-confirm flag${shouldConfirmDestructive ? ' and --allow-destructive flag' : ''}. Exiting`));
        return Err('is_ci');
    }
    try {
...
function summaryMessageColumns({ name, newItems, updatedItems, deleteItems }) {
    return {
        name: ` ↳ ${name}`,
...
function shortSummaryMessage({ name, newItems, deleteItems }) {
