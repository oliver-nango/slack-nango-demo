...
import { DuplicateEndpointDefinitionError, DuplicateModelDefinitionError, EndpointMismatchDefinitionError, InvalidIntervalDefinitionError, InvalidModelDefinitionError, TrackDeletesDefinitionError } from './utils.js';
const allowed = ['action', 'sync', 'onEvent'];
export async function buildDefinitions({ fullPath, debug }) {
    const parsed = { yamlVersion: 'v2', integrations: [], models: new Map() };
    ...
    for (const filePath of matched) {
        ...
        const script = moduleContent.default.default;
        ...
        let integration = parsed.integrations.find((v) => v.providerConfigKey === integrationId);
        if (!integration) {
            integration = {
...
export function buildSync({ filePath, params, integrationIdClean, basename, basenameClean }) {
    ...
    const sync = {
        ...
        output: Object.entries(params.models).map(([name, model]) => {
            ...
            models.set(name, { name, fields: to['value'], description: to.description });
            usedModels.add(name);
            return name;
        }),
...
export function buildAction({ params, integrationIdClean, basename, basenameClean }) {
    const models = new Map();
...
function postValidation(parsed) {
    for (const integration of parsed.integrations) {
        ...
        for (const sync of integration.syncs) {
            ...
            for (const endpoint of sync.endpoints) {
                ...
                if (seenEndpoints.has(key)) {
                    return Err(new DuplicateEndpointDefinitionError(key, `${integration.providerConfigKey}/syncs/${sync.name}.ts`, ['createSync', 'endpoints']));
                }
        ...
        for (const action of integration.actions) {
            if (action.endpoint) {
                ...
                if (seenEndpoints.has(key)) {
                    return Err(new DuplicateEndpointDefinitionError(key, `${integration.providerConfigKey}/actions/${action.name}.ts`, ['createAction', 'endpoint']));
                }
                seenEndpoints.add(key);
            }
        }
