 ...
 */
export async function compileAll({ fullPath, debug }) {
 ...
 */
export async function readIndexContent(fullPath) {
 ...
 */
export function getEntryPoints(indexContent) {
    const entryPoints = [];
    let match;
 ...
 */
function typeCheck({ fullPath, entryPoints }) {
 ...
 */
export async function bundleFile({ entryPoint, projectRootPath }) {
    ...
    try {
        ...
        if (bag.deleteRecordsFromPreviousExecutionsLines.length > 0 &&
 ...
 */
export async function compileOne({ entryPoint, projectRootPath }) {
...
export function tsToJsPath(filePath) {
 ...
 */
function nangoPlugin({ entryPoint }) {
    const proxyLines = [];
    const batchingRecordsLines = [];
    const setMergingStrategyLines = [];
    const deleteRecordsFromPreviousExecutionsLines = [];
    const bag = {
        proxyLines,
                ...
                visitor: {
                    ImportDeclaration(path) {
                        ...
                        if (!allowedPackages.includes(source) && kind !== 'type') {
                            throw new CompileError('disallowed_import', lineNumber, `Import of package '${source}' is not allowed. Allowed packages are: ${allowedPackages.join(', ')}`);
                        }
                    },
                    CallExpression(astPath) {
                        ...
                        const isThenOrCatch = astPath.findParent((parentPath) => t.isMemberExpression(parentPath.node) &&
                        ...
                        if (!isAwaited && !isThenOrCatch && !isReturned && needsAwait.includes(callee.property.name)) {
                            throw new CompileError('method_need_await', lineNumber, `nango.${callee.property.name}() calls must be awaited in. Not awaiting can lead to unexpected results.`);
                        }
                        ...
                        const currentFilePath = astPath.hub?.file?.opts?.filename;
                        if (currentFilePath) {
                    ...
                    ExportNamedDeclaration(astPath) {
                        ...
                        if (astPath.node.__transformedByRemoveCreateWrappers) {
                            return;
                        }
                        ...
                        const namedExportError = (exportedName) => {
                            throw new CompileError('nango_named_export_not_allowed', lineNumber, `Named export '${exportedName}' is not allowed. Only export default and ${allowedExports.join(', ')} are permitted.`);
                        };
                    ...
                    ExportDefaultDeclaration(astPath) {
                        ...
                        if (t.isCallExpression(decl) && t.isIdentifier(decl.callee) && allowedExports.includes(decl.callee.name)) {
                            ...
                            if (calleeName === 'createAction')
                                varName = 'action';
                            if (calleeName === 'createSync')
                                varName = 'sync';
                            if (calleeName === 'createOnEvent')
                                varName = 'onEvent';
                            ...
                            exportConst.__transformedByRemoveCreateWrappers = true;
                            exportDefault.__transformedByRemoveCreateWrappers = true;
