...
export async function migrateToZeroYaml({ fullPath, debug }) {
 ...
 * Get script content
 */
async function getContent({ targetFile }) {
 ...
 */
export async function runPackageManagerInstall(fullPath) {
 ...
 */
function buildExecProp(j, func, execReturnType) {
    let params;
    let bodyNode;
    let isAsync = false;
    ...
    if (execReturnType) {
        execArrow.returnType = j.tsTypeAnnotation(j.tsTypeReference(j.identifier('Promise'), j.tsTypeParameterInstantiation([j.tsTypeReference(j.identifier(execReturnType))])));
    }
 ...
 */
export function transformSync({ content, sync, models }) {
    ...
    root.find(j.ExportDefaultDeclaration).forEach((path) => {
        ...
        const leadingComments = func.leadingComments || path.node.leadingComments;
        ...
        const endpointsProp = j.objectProperty(j.identifier('endpoints'), j.arrayExpression(sync.endpoints.map((ep) => j.objectExpression(Object.entries(ep)
            .map(([k, v]) => {
            if (typeof v === 'string') {
        ...
        if (sync.webhookSubscriptions.length > 0) {
            const webhookSubscriptionsProp = j.objectProperty(j.identifier('webhookSubscriptions'), Array.isArray(sync.webhookSubscriptions) ? j.arrayExpression(sync.webhookSubscriptions.map((w) => j.stringLiteral(w))) : j.arrayExpression([]));
        ...
        if (sync.input) {
            ...
            const metadataProp = j.objectProperty(j.identifier('metadata'), j.callExpression(j.memberExpression(j.identifier('z'), j.identifier('object')), [j.objectExpression([])]));
            props.push(metadataProp);
        }
        // Add exec prop
        props.push(execProp);
        ...
        root.find(j.ExportNamedDeclaration).forEach((p) => {
            ...
            if (decl && decl.type === 'FunctionDeclaration' && decl.id && decl.id.name === 'onWebhookPayloadReceived') {
                ...
                // Capture comments
                onWebhookComments = decl.leadingComments || p.node.leadingComments;
        ...
        if (onWebhookArrow) {
            ...
            if (onWebhookComments) {
                onWebhookProp.comments = onWebhookComments;
            }
 ...
 */
export function transformAction({ content, action, models }) {
    const j = jscodeshift.withParser('ts');
    const root = j(content);
    removeLegacyImports({ root, j });
    ...
    root.find(j.ExportDefaultDeclaration).forEach((path) => {
        ...
        if (action.endpoint) {
            const endpointProp = j.objectProperty(j.identifier('endpoint'), j.objectExpression(Object.entries(action.endpoint)
                .map(([k, v]) => {
                if (typeof v === 'string') {
                    return j.objectProperty(j.identifier(k), j.stringLiteral(v));
                }
        ...
        const inputProp = j.objectProperty(j.identifier('input'), typeof action.input === 'string' ? j.identifier(action.input) : j.callExpression(j.memberExpression(j.identifier('z'), j.identifier('void')), []));
 ...
 */
export function transformOnEvents({ content, eventType, models }) {
    const j = jscodeshift.withParser('ts');
    const root = j(content);
    removeLegacyImports({ root, j });
 ...
 */
function removeLegacyImports({ root, j }) {
 ...
 */
function reImportTypes({ root, j, usedModels }) {
    // Find all used allowedTypesImports in the file
    const usedAllowedTypes = new Set();
    ...
    if (usedAllowedTypes.size > 0) {
        const importTypeDecl = j.importDeclaration(Array.from(usedAllowedTypes).map((type) => j.importSpecifier(j.identifier(type))), j.literal('nango'));
        importTypeDecl.importKind = 'type';
        importDecls.push(importTypeDecl);
    }
    if (usedModelTypes.size > 0) {
        importDecls.push(j.importDeclaration([...Array.from(usedModelTypes.values()).map((name) => j.importSpecifier(j.identifier(name)))], j.literal('../../models.js')));
    }
    // Import z if any z.* is used (e.g., z.never())
    const usesZ = root.find(j.Identifier, { name: 'z' }).size() > 0;
    const hasZodImport = root
 ...
 */
async function addPackageJson({ fullPath, debug }) {
 ...
 */
async function generateIndexTs({ fullPath, parsed }) {
    const indexLines = [];
    for (const integration of parsed.integrations) {
 ...
 */
function removeBatchTypeArguments({ root, j }) {
 ...
 */
export function generateModelsTs({ parsed }) {
    const j = jscodeshift.withParser('ts');
    const root = j('');
    ...
    for (const modelName of sortedModels) {
        ...
        root.get().node.program.body.push(j.exportNamedDeclaration(j.variableDeclaration('const', [j.variableDeclarator(j.identifier(modelName), nangoModelToZod({ j, model, referencedModels: allModelNames }))]), []));
        // Exported type alias
        root.get().node.program.body.push(j.exportNamedDeclaration(j.tsTypeAliasDeclaration(j.identifier(modelName), j.tsTypeReference(j.tsQualifiedName(j.identifier('z'), j.identifier('infer')), j.tsTypeParameterInstantiation([j.tsTypeQuery(j.identifier(modelName))]))), []));
    }
    // Export all models
    root.get().node.program.body.push(j.exportNamedDeclaration(j.variableDeclaration('const', [
        j.variableDeclarator(j.identifier('models'), j.objectExpression(sortedModels.map((modelName) => j.objectProperty(j.identifier(modelName), j.identifier(modelName)))))
    ])));
 ...
 */
export function nangoModelToZod({ j, model, referencedModels }) {
...
function nangoTypeToZodAst({ j, field, referencedModels }) {
    ...
    if (field.array) {
        ...
        if (!Array.isArray(field.value)) {
            return j.callExpression(j.memberExpression(nangoTypeToZodAst({ j, field: { ...field, array: false }, referencedModels }), j.identifier('array')), []);
        }
    ...
    if (typeof field.value === 'string') {
        switch (field.value) {
            ...
            case 'Record<string, any>':
                ...
                break;
            case 'any[]':
                baseExpr = j.callExpression(j.memberExpression(j.callExpression(j.memberExpression(j.identifier('z'), j.identifier('any')), []), j.identifier('array')), []);
                break;
            default:
...
// Topological sort
function topoSortModels(modelNames, models) {
...
function createNangoLocalType({ j, name, variable }) {
    return j.exportNamedDeclaration(j.tsTypeAliasDeclaration(j.identifier(name), j.tsIndexedAccessType(j.tsTypeReference(j.identifier('Parameters'), j.tsTypeParameterInstantiation([j.tsIndexedAccessType(j.tsTypeQuery(j.identifier(variable)), j.tsLiteralType(j.stringLiteral('exec')))])), j.tsLiteralType(j.numericLiteral(0)))));
}
// Helper: For each file in the list, update model imports for NangoSync/NangoAction
async function processHelperFiles({ fullPath, parsed }) {
...
export function processHelperFile({ content }) {
    ...
    root.find(j.ImportDeclaration).forEach((pathNode) => {
        ...
        const nangoSpecifiers = specifiers.filter((s) => isImportSpecifier(s) && allowedTypesImportsHelper.includes(s.imported.name));
        if (nangoSpecifiers.length === 0) {
            return;
        }
        // Remove NangoSync/NangoAction from models import
        pathNode.node.specifiers = specifiers.filter((s) => !(isImportSpecifier(s) && allowedTypesImportsHelper.includes(s.imported.name)));
        changed = true;
        ...
        if (nangoImport.size() > 0) {
            ...
            // Add new import
            root.get().node.program.body.unshift(j.importDeclaration(nangoSpecifiers
                .filter(isImportSpecifier)
                .filter((s) => typeof s.imported.name === 'string')
                .map((s) => {
                const name = s.imported.name;
...
async function hasSymlinkInPath(filePath, stopAtPath) {
...
//# sourceMappingURL=toZeroYaml.js.map