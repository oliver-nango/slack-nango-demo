...
class DeployService {
    async prep({ fullPath, options, environment, debug = false }) {
        ...
        if (hasSingleScript && integrationIdMode) {
            const scriptName = String(optionalSyncName || optionalActionName);
        ...
        try {
            ...
            for (const sync of newSyncs) {
                const syncMessage = sync.connections === 0 || sync.auto_start === false
            ...
            if (deletedModels.length > 0) {
                console.log(chalk.red(`The following models have been removed: ${deletedModels.join(', ')}. WARNING: Renaming a model is the equivalent of deleting the old model and creating a new one. Records from the old model won't be transferred to the new model. Consider running a full sync to transfer records.`));
            }
            // force confirmation :
    ...
    async deploy(url, body) {
        await http
            ...
            .catch((err) => {
            const errorMessage = err instanceof AxiosError ? JSON.stringify(err.response?.data, null, 2) : JSON.stringify(err, ['message', 'name', 'stack'], 2);
            console.log(chalk.red(`Error deploying the functions with the following error: ${errorMessage}`));
            process.exit(1);
        });
    }
    async internalDeploy({ fullPath, environment, options, debug = false }) {
        ...
        const bodyDeploy = { ...postData, reconcile: true, debug, nangoYamlBody, singleDeployMode: false };
        await this.deploy(url, bodyDeploy);
    }
    package({ parsed, fullPath, debug, version = '', optionalSyncName, optionalActionName, integrationId }) {
        if (integrationId) {
            if (debug) {
...
function loadScriptFiles({ fullPath, scriptName, providerConfigKey, type }) {
...
function loadScriptJsFile({ scriptName, providerConfigKey, fullPath }) {
...
function loadScriptTsFile({ fullPath, scriptName, providerConfigKey, type }) {
