...
export async function generate({ absolutePath, path, parsed, isForIntegrationTemplates = false, debug = false }) {
    ...
    for (const config of integrations) {
        ...
        const toGenerate = [...Object.values(config.syncs), ...Object.values(config.actions)];
...
function updateReadme({ markdown, scriptName, scriptPath, endpointType, scriptConfig, models, isForIntegrationTemplates }) {
...
function generalInfo(scriptPath, endpointType, scriptConfig, isForIntegrationTemplates) {
    ...
    const inputModelList = endpointType === 'action' && scriptConfig.input ? (Array.isArray(scriptConfig.input) ? scriptConfig.input : [scriptConfig.input]) : [];
...
function requestEndpoint(scriptConfig) {
...
function requestParams(endpointType) {
    const out = ['### Request Query Parameters'];
    if (endpointType === 'sync') {
        out.push(``, `- **modified_after:** \`(optional, string)\` A timestamp (e.g., \`2023-05-31T11:46:13.390Z\`) used to fetch records modified after this date and time. If not provided, all records are returned. The modified_after parameter is less precise than cursor, as multiple records may share the same modification timestamp.`, `- **limit:** \`(optional, integer)\` The maximum number of records to return per page. Defaults to 100.`, `- **cursor:** \`(optional, string)\` A marker used to fetch records modified after a specific point in time.If not provided, all records are returned.Each record includes a cursor value found in _nango_metadata.cursor.Save the cursor from the last record retrieved to track your sync progress.Use the cursor parameter together with the limit parameter to paginate through records.The cursor is more precise than modified_after, as it can differentiate between records with the same modification timestamp.`, `- **filter:** \`(optional, added | updated | deleted)\` Filter to only show results that have been added or updated or deleted.`, `- **ids:** \`(optional, string[])\` An array of string containing a list of your records IDs. The list will be filtered to include only the records with a matching ID.`, ``);
    }
    else {
...
function requestBody(scriptConfig, endpointType, models) {
...
function requestResponse(scriptConfig, models) {
...
function expectedMetadata(scriptConfig, endpointType, models) {
 ...
 */
export function modelToJson({ model, models, required = [] }) {
    ...
    if ('enum' in model && Array.isArray(model.enum)) {
        const enumVals = model.enum.map((v) => `'${String(v)}'`);
    ...
    if ('type' in model && typeof model.type === 'string') {
        switch (model.type) {
            ...
            case 'array': {
                ...
                if (items) {
                    const itemExample = modelToJson({ model: items, models, required });
