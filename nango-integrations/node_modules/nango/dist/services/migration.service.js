...
export const v1toV2Migration = (loadLocation) => {
...
async function createDirectory(dirPath, debug = false) {
...
async function moveFile(source, destination, debug = false) {
...
async function updateModelImport(filePath, debug = false) {
    try {
...
export const directoryMigration = async (loadLocation, debug) => {
    const parsing = parse(loadLocation);
    if (parsing.isErr()) {
        return;
    }
    ...
    for (const integration of parsing.value.parsed.integrations) {
        ...
        if (integration.syncs) {
            ...
            for (const sync of integration.syncs) {
                const syncPath = path.join(syncsPath, `${sync.name}.ts`);
        ...
        if (integration.actions) {
            ...
            for (const action of integration.actions) {
                const actionPath = path.join(actionsPath, `${action.name}.ts`);
...
export function endpointMigration(loadLocation) {
    const parsing = parse(loadLocation);
    if (parsing.isErr()) {
        return;
    }
    ...
    let dump = parsing.value.yaml.replace(/^(\s+)endpoint: ((GET|POST|PUT|PATCH|DELETE)\s)?(\/[a-zA-Z0-9-:{}./_]+)$/gim, `$1endpoint:\r\n$1  method: $3\r\n$1  path: $4`);
