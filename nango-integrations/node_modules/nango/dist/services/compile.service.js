...
function getCachedParser({ fullPath, debug }) {
...
export async function compileAllFiles({ debug, fullPath, scriptName, providerConfigKey, type }) {
...
export async function compileSingleFile({ fullPath, file, tsconfig, parsed, debug = false }) {
...
function compileImportedFile({ fullPath, filePath, compiler, parsed, type }) {
    ...
    for (const importedFile of importedFiles) {
        ...
        if (importedFilePathResolved.split(path.sep).length < fullPath.split(path.sep).length) {
            ...
            console.log(chalk.red(`All imported files must live within the nango-integrations directory. Please move "${importedFileName}" into the nango-integrations directory.`));
            return false;
        }
...
async function compile({ fullPath, file, compiler, cachedParser, debug = false }) {
    const parsed = cachedParser();
    if (!parsed) {
        return false;
    }
    ...
    const onEventScripts = Object.entries(providerConfiguration.onEventScripts).flatMap(([event, scripts]) => scripts.map((name) => ({ name, type: 'on-event', event })));
...
export function getFileToCompile({ fullPath, filePath }) {
...
export function resolveTsFileLocation({ fullPath, scriptName, providerConfigKey, type }) {
...
export function listFilesToCompile({ fullPath, scriptDirectory, scriptName, parsed, debug, providerConfigKey }) {
    let files = [];
    ...
    // Filtering by providerConfigKey or compiling all files
        ...
        for (const integration of filteredIntegrations) {
            ...
            const scriptTypes = ['syncs', 'actions', 'on-events', 'post-connection-scripts'];
