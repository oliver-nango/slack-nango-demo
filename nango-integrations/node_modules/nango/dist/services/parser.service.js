...
class ParserService {
    ...
    callsAreUsedCorrectly(filePath, type = 'sync', modelNames) {
        let areAwaited = true;
            ...
            ExportDefaultDeclaration(path) {
                ...
                if (t.isFunctionDeclaration(declaration) || t.isFunctionExpression(declaration) || t.isArrowFunctionExpression(declaration)) {
                    if (functionReturnsValue(declaration) && type === 'sync') {
                        ...
                        console.log(chalk.red(`The default exported function fetchData at "${filePath}:${lineNumber}" must not return a value. Sync functions should not return but rather use batchSave to save data.`));
                        noReturnUsed = false;
                    }
                }
            }
        });
        if (batchingRecordsLines.length > 0 &&
            setMergingStrategyLines.length > 0 &&
            setMergingStrategyLines.some((line) => line > Math.min(...batchingRecordsLines))) {
            console.log(chalk.red(`setMergingStrategy should be called before any batching records function in "${filePath}:${Math.min(...setMergingStrategyLines)}".`));
            usedCorrectly = false;
        }
        ...
        if (deleteRecordsFromPreviousExecutionsLines.length > 1) {
            console.log(chalk.red(`deleteRecordsFromPreviousExecutions should be called only once in "${filePath}:${Math.max(...deleteRecordsFromPreviousExecutionsLines)}".`));
            usedCorrectly = false;
        }
        if (batchingRecordsLines.length > 0 &&
            ...
            console.log(chalk.red(`deleteRecordsFromPreviousExecutions should be called after all batching records functions in "${filePath}:${Math.max(...deleteRecordsFromPreviousExecutionsLines)}".`));
            usedCorrectly = false;
        }
