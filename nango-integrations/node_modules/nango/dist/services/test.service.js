 ...
 */
export function validateAndFilterIntegrations({ integrations, integrationId, syncName, actionName }) {
 ...
 */
export function shouldProcessSync({ currentSyncName, syncName, actionName }) {
 ...
 */
export function shouldProcessAction({ currentActionName, syncName, actionName }) {
...
async function findUpFilename(filename, fromDir) {
...
async function fetchLatestVersions(packages, debug) {
    const versions = {};
...
async function injectTestDependencies({ debug }) {
...
export async function generateSyncTest({ integration, syncName, modelName, writePath, debug }) {
    const data = {
        integration,
        syncName,
        modelName
...
export async function generateActionTest({ integration, actionName, output, writePath, debug }) {
    const data = {
        integration,
        actionName,
        output
...
async function generateTestConfigs({ debug, force = false }) {
    try {
        const rootPath = await getProjectRoot();
        if (debug) {
        ...
        if (force || !(await pathExists(viteConfigPath))) {
            ...
            if (debug)
                printDebug(`Created/Overwritten vite.config.ts at ${viteConfigPath}`);
        }
        else if (debug) {
        ...
        if (force || !(await pathExists(vitestSetupPath))) {
            ...
            if (debug)
                printDebug(`Created/Overwritten vitest.setup.ts at ${vitestSetupPath}`);
        }
        else if (debug) {
...
export async function generateTests({ absolutePath, integrationId, syncName, actionName, debug = false, autoConfirm = false }) {
    try {
        if (debug) {
