...
export class DryRunService {
    ...
    environment;
    returnOutput;
    constructor({ environment, returnOutput = false, fullPath, validation, isZeroYaml }) {
    ...
    async run(options, debug = false) {
        ...
        if ((!scriptInfo && !isOnEventScript) || !providerConfigKey) {
            console.log(chalk.red(`No script matched "${syncName}"${options.optionalProviderConfigKey ? ` for integration "${options.optionalProviderConfigKey}"` : ''}`));
            return;
        }
        if (debug && scriptInfo) {
            printDebug(`Found integration ${providerConfigKey}, ${scriptInfo.type} ${scriptInfo.name} `);
        }
        const nangoConnection = await getConnection(providerConfigKey, connectionId, {
            'Nango-Is-Sync': true,
        ...
        try {
            ...
            if (options.saveResponses) {
                nangoProps.axios = {
                    response: {
                        onFulfilled: (response) => responseSaver.onAxiosRequestFulfilled({
                            response,
                            providerConfigKey,
                            connectionId: nangoConnection.connection_id,
                            syncName,
                        ...
                        onRejected: (error) => responseSaver.onAxiosRequestRejected({
                            error,
                            providerConfigKey,
                            connectionId: nangoConnection.connection_id,
                            syncName,
    ...
    async runScript({ syncName, nangoProps, loadLocation, input, stubbedMetadata, diagnostics }) {
        ...
        const nango = nangoProps.scriptType === 'sync' || nangoProps.scriptType === 'webhook'
        ...
        try {
            ...
            try {
                        ...
                        switch (moduleName) {
                            ...
                            case 'botbuilder':
                                if (!preloaded[moduleName]) {
                                    throw new Error(`Module '${moduleName}' needs to be installed on your side in a package.json. You can also test using the Nango dashboard`);
                                }
                ...
                if (isAction) {
                    ...
                    const modelNameOutput = nangoProps.syncConfig.models && nangoProps.syncConfig.models.length > 0 ? nangoProps.syncConfig.models[0] : undefined;
                ...
                if (typeof scriptExports.default !== 'function') {
                    ...
                    const results = await payload.exec(nango);
...
function getIntegrationFile({ syncName, location, nangoProps, isZeroYaml }) {
    try {
        if (isZeroYaml) {
...
//# sourceMappingURL=dryrun.service.js.map